...
- **面向 AI。** 开箱即用的 [`AGENTS.md`](AGENTS.md)、确定性自然语言检索 CLI（`npx @voltwake/animefx --query`）与紧凑的 `manifest/ai-catalog.json`，让编程代理找到最接近的真实效果，而不是凭空捏造。
...
```bash
npx @voltwake/animefx --query "高级但克制的标题揭晓" --limit 3
```
...
```text
在本项目中使用 animefx（没装先 npm install @voltwake/animefx）：读取 node_modules/@voltwake/animefx/AGENTS.md，
...
- 运行时、效果数据、AI 技能、工具与 Demo 代码（即 npm 包 [`@voltwake/animefx`](https://www.npmjs.com/package/@voltwake/animefx) 分发的一切）按 [MIT License](LICENSE) 授权——个人与商业项目均可免费使用、修改、分发，仅需随分发保留版权声明（第三方声明见 [`licenses/`](licenses/)）。
